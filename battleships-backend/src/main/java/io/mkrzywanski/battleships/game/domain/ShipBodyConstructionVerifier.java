...
final class ShipBodyConstructionVerifier {
    ...
    private static void verifyContinuityInAxis(final List<Position> positions, final Function<Position, Integer> axisValueExtractor) {
        ...
        while (iterator.hasNext()) {
            ...
            if (previous + 1 != next) {
                throw new IllegalShipPartPositionException("Discontinuity detected");
            }
    ...
    private static boolean isVertical(final List<Position> positions) {
        return checkCoordinateContinuity(positions, Position::x);
    }

    private static boolean isHorizontal(final List<Position> positions) {
        return checkCoordinateContinuity(positions, Position::y);
    }

    private static <T> boolean checkCoordinateContinuity(final List<Position> positions,
                                                         final Function<Position, Integer> extractor) {
